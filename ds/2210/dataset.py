            ...
            for i in range(len(self.data_list)):
                ...
                if audio_multi_turn:
                    if self.data_list[i][3] and self.data_list[i][3][-1]:
                        self.data_list[i] = [np.load(f"{root}/{task}/raw/{self.data_list[i][0]}"), temp, self.data_list[i][2], 
                                             token_type, np.load(f"{root}/{task}/raw/{self.data_list[i][3][-1]}")]
                    else:
                        self.data_list[i] = [np.load(f"{root}/{task}/raw/{self.data_list[i][0]}"), temp, self.data_list[i][2], token_type, []]
                else:
                    self.data_list[i] = [np.load(f"{root}/{task}/raw/{self.data_list[i][0]}"), temp, self.data_list[i][2], token_type]
        else:
...
class DataCollatorForDownstream:
    def __init__(self, audio_length, float_label, prompt=False):
