...
class WavLMForMultiTurn(WavLMPreTrainedModel):
    ...
    def forward(self, input_values, attention_mask, perform_mam=False, token_embedding=None):
        ...
        if self.config.train_mode == 3:
            ...
            token_type_id = torch.ones([bs * 2, new_len], device=aa.device, dtype=torch.long)
            masked_indices_for_concat = torch.zeros([bs, new_len], device=aa.device, dtype=masked_indices.dtype)
            for i in range(bs):
                ...
                masked_indices_for_concat[i, :pl] = torch.cat([torch.tensor([0]).bool().to(aa.device), masked_indices[3 * i][:laa, 0], torch.tensor([0]).bool().to(aa.device), masked_indices[3 * i + 1][:lpa, 0]], dim=0)
            ...
            for i in range(bs):
                ...
                if perform_mam:
                    masked_indices_for_concat[i, :pl] = torch.cat([torch.tensor([0]).bool().to(a1.device), masked_indices[2 * i][:la1, 0], torch.tensor([0]).bool().to(a1.device), masked_indices[2 * i + 1][:la2, 0]], dim=0)
            if perform_mam:
        ...
        return hidden_states, attention_mask, None, None
