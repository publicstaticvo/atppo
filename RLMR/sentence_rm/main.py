...
from util import *
from trainer_sentence import SentenceAlignTrainer
...
if __name__ == "__main__":
    ...
    c = DataCollatorForSingleTurnSRM(tokenizer, config, args.apex_level > 0)
    if args.model_path:
        model = SentenceAlignTrainer.from_pretrained(args.model_path, config=config)
    else:
        model = SentenceAlignTrainer(config, args.audio_path, args.text_path)
    # 6。数据并行
