...
from util import *
from trainer_word import WordAlignTrainer
...
if __name__ == "__main__":
    ...
    c = DataCollatorForSingleTurnWRM(tokenizer, config, args.apex_level > 0)
    if args.model_path:
        model = WordAlignTrainer.from_pretrained(args.model_path, config=config)
    else:
        model = WordAlignTrainer(config, args.audio_path, args.text_path)
    # 6。数据并行
