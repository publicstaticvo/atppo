...
class ATModel(PreTrainedModel):
    ...
    def forward(self, audio_input, text_input=None, audio_attention_mask=None, text_attention_mask=None,
                role_token_id=None, turn_id=None, mlm_label=None):
...
class DSTModel(nn.Module):
    ...
    def forward(self, text_input, text_mask, role_token_id, audio_input, audio_mask, turn_id=None,
        ...
        sequence_output, pooled_output = self.model(audio_input, text_input, audio_mask, text_mask, role_token_id,
                                                    turn_id)  # last hidden state (b, l, 768)
        ...
        if diag_state is not None:
            diag_state_labels = torch.clamp(torch.stack(list(diag_state.values()), 1).float(), 0.0,
                                            1.0)  # (b, len(class_types)) 表示slot是否有状态
        ...
        for slot in self.slot_list:
            if self.class_aux_feats_inform and self.class_aux_feats_ds:
                pooled_output_aux = torch.cat(
                    (pooled_output, self.inform_projection(inform_labels), self.ds_projection(diag_state_labels)),
                    1)  # (b, 768+30+30)
